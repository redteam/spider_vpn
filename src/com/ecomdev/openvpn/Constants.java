package com.ecomdev.openvpn;

/**
 * Created by ymka on 09.08.14.
 */
public class Constants {

    public static final int UPDATE_DEMO_TIME_RECEIVER_NUM = 24442;
    public static final String PREF_NEED_TO_VERIFICATION = "needToVerification";
    public static final String PREF_LEFT_TIME = "leftTime";
    public static final String PREF_IS_DEMO = "isDemo";
    public static final String PREF_IS_DEMO_RUNNING = "isDemoRunning";
    public static final String PREF_IS_TIMEOUT = "isTimeOut";
    public static final String PREF_LEFT_HOURS = "leftHours";
    public static final String PREF_USER_LOGGED = "userLogged";

    public static final String TIMER_MESSAGE = "timer_message";
    public static final String LEFT_HOURS = "left_hours";

    public static final int ONE_HOUR = 1000 * 60 * 60; // milliseconds in an hour

}
